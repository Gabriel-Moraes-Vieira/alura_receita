...
def cadastro(request):
    if request.method == 'POST':
        ...
        if campo_vazio(nome):
            messages.error(request,'É necessário preencher o campo nome')
            return redirect('cadastro')
        if campo_vazio(email):
            messages.error(request, 'É necessário preencher o campo email')
            return redirect('cadastro')
...
def login(request):
    if request.method == 'POST':
        ...
        if campo_vazio(email) or campo_vazio(senha):
            messages.error(request,'O campo Email e Senha devem ser preenchidos')
        if User.objects.filter(email=email).exists():
